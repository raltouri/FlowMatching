...
# --- Figure 5: how far the flow strays from the path it was taught ------


def fig_path_drift() -> None:
    from src.analysis_stage2 import path_drift

    k, seed = REPRESENTATIVE[2], REPRESENTATIVE[3]
    fig, axes = plt.subplots(1, 3, figsize=(12.5, 3.6), sharey=True)

    for ax, (dataset, encoder) in zip(axes, config.PIPELINES):
        for method, color in (("fm_std_T4", BLUE), ("fm_roll_T4", ORANGE)):
            t, drift, _ = path_drift(dataset, encoder, method, k, seed)
            # Drift is exactly zero at t=0 by construction, which a log axis
            # cannot show; the curve starts at the first Euler step.
            ax.plot(t[1:], drift[1:], color=color, linewidth=2, marker="o",
                    markersize=4, label=method)
        # Unit-normalised features: two of them can be at most 2 apart, so
        # anything above this line is outside the region real features occupy.
        ax.axhline(2.0, color=MUTED, linewidth=0.8, linestyle="--")
        ax.set_yscale("log")
        ax.set_xlabel("t (fraction of the journey)")
        ax.set_title(f"{dataset} · {ENCODER_LABEL[encoder]}")

    axes[0].set_ylabel("distance from the ideal path")
    axes[0].annotate("max distance between\ntwo unit features", (0.27, 2.15),
                     color=MUTED, fontsize=7)
    axes[0].legend(loc="lower right")
    fig.suptitle(
        "Standard FM roughly follows the straight line it was trained on; rolled-out "
        "leaves the feature space entirely\n"
        f"K={k}, seed {seed}, T=4. Distance to the ideal interpolation at each step, "
        "averaged over the test split.",
        fontsize=10, color=INK, y=1.13,
    )
    save(fig, "path_drift.png")
